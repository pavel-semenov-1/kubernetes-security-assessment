...
Kube-bench does not require an explicit installation process.

## Usage

To scan a Kubernetes namespace we can simply apply a job resource:

``` {.bash}
kubectl apply -f https://github.com/aquasecurity/kube-bench/raw/refs/heads/main/job.yaml
```

Then, we get the scan results from the pod logs:

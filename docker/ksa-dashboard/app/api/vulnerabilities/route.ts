...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_PARSER_API_URL}/vulnerabilities?reportId=${reportId}&search=${search || ""}`, {
      method: "GET"
    });
    if (!response.ok) {
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_PARSER_API_URL}/vulnerabilities?reportId=${reportId}&search=${search || ""}&id=${id || ""}`, {
      method: "DELETE",
    });
